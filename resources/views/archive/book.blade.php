                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <button type="button" onclick="restoreBook({{ $item->id }})"
                                                class="dropdown-item"><i class='bx bx-redo'></i>{{ __('Restore') }}
                                            </button>
                                            <button type="button" onclick="forceDelete({{ $item->id }},this)"
                                                class="dropdown-item"><i
                                                    class="bx bx-trash me-2"></i>{{ __('Permanent delete') }}</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
<script>
    function forceDelete(id, refranses) {
        ...
        deleteItem('/book/forceDelete/', id, refranses)
    ...
    function restoreBook(id) {
        ...
        axios.post('/book/restore/' + id)
            .then(function(response) {
                success(response.data)
                window.location.href = '/book/archive'
